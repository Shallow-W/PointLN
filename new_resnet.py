    ...
    def forward(self, input):
        ...
        for branch in self.branches:
            ...
            for j in range(2 * self.B):
                ...
                # 应该是对pij进行sigmoid
                out1 = torch.sigmoid(out[:,(23 + 2 * self.S) * j, ...]).unsqueeze(
                    1)  # ([batch, H, W]) -> ([batch,1, H, W])
                # 对xij，yij进行sigmoid
                out2 = torch.sigmoid(
...
def inceptionresnetv2(num_classes=20, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.load("E:\学习\Pycharm\文件\CV\learn\PLN2\inceptionresnetv2-520b38e4.pth")
...
if __name__ == '__main__':
    ...
    model = inceptionresnetv2(num_classes=20, pretrained='imagenet')
    print(model)
